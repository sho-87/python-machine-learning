...
if GRID_SEARCH:
    ...
    for i, w in enumerate(search_widths):
        ...
        cur_test_acc, weights = main(batch_size=2000, num_epochs=20, k_width=w)
    ...
    cur_test_acc, weights = main(batch_size=2000, num_epochs=100,
                        k_width=search_widths[best_idx[1]])
    
    # Final test accuracy: NA
else:
    cur_test_acc, weights = main(batch_size=2000, num_epochs=100, k_width=11)  # 88.21%


# Get learned filters
c1_kernels = weights[0].reshape((-1,1,1,11))

# Create plot
fig, axarr = plt.subplots(16, 3, figsize=(9, 10))
axarr[0,0].set_title('On Task', fontsize=12, y=1.3)
axarr[0,1].set_title('Kernels', fontsize=12, y=1.1)
axarr[0,2].set_title('Mind Wandering', fontsize=12, y=1.3)

# Plot layer 1 kernels
for j in range(len(c1_kernels)):
    axarr[j,1].imshow(c1_kernels[j,0,0].reshape((1,11)), cmap=plt.cm.bone, interpolation='none')
    axarr[j,1].get_xaxis().set_visible(False)
    axarr[j,1].get_yaxis().set_visible(False)

ot_idx = np.where(data_labels==0)[0][0]
mw_idx = np.where(data_labels==1)[0][0]

# Plot activation maps (OT)
x = T.tensor4().astype(theano.config.floatX)
conv_out = T.nnet.conv2d(input=x, filters=c1_kernels, border_mode='half')

get_activity = theano.function([x], conv_out)
activation = get_activity(data[ot_idx,0,:].reshape((1,1,1,30)))

for j in range(len(c1_kernels)):
    axarr[j,0].imshow(activation[0,j,0].reshape((1,30)), cmap=plt.cm.gray, interpolation='none')
    axarr[j,0].get_xaxis().set_visible(False)
    axarr[j,0].get_yaxis().set_visible(False)
    
# Plot activation maps (MW)
x = T.tensor4().astype(theano.config.floatX)
conv_out = T.nnet.conv2d(input=x, filters=c1_kernels, border_mode='half')

get_activity = theano.function([x], conv_out)
activation = get_activity(data[mw_idx,0,:].reshape((1,1,1,30)))

for j in range(len(c1_kernels)):
    axarr[j,2].imshow(activation[0,j,0].reshape((1,30)), cmap=plt.cm.gray, interpolation='none')
    axarr[j,2].get_xaxis().set_visible(False)
    axarr[j,2].get_yaxis().set_visible(False)
